...
from app.models.database import get_profile, get_active_services, get_user_orders, get_service_by_id, create_order, debit_balance
...
def index():
    ...
    form = OrderForm()
    return render_template("dashboard/index.html", user=user, profile=profile, services_by_network=services_by_network, orders=orders, form=form, whatsapp=current_app.config["WHATSAPP_NUMBER"], currency=session.get("currency", "FCFA"))
...
def place_order():
    ...
    if balance < total_price:
        ...
        return redirect(url_for("dashboard.index"))
    order_data = {"user_id": user["id"], "user_email": user["email"], "reseau": service["reseau"], "service": service["categorie"], "service_id": service["id"], "quantite": quantity, "lien": link, "prix_unitaire": unit_price, "prix_total": total_price, "statut": "en_attente", "progression": 0}
    ...
    if new_balance is None:
        ...
        return redirect(url_for("dashboard.index"))
    flash(f"Commande passee ! {total_price:,.0f} FCFA deduits.", "success")
...
def api_services(network):
    ...
    return jsonify([{"id": s["id"], "categorie": s["categorie"], "prix_fcfa": s["prix_fcfa"], "min_qte": s["min_qte"], "max_qte": s["max_qte"], "description": s.get("description", "")} for s in services])
